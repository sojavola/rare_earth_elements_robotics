...
class REEExplorationServer(Node):
    ...
    def clean_area(self, x, y):
        """Nettoie une zone autour de la position"""
        cleaning_radius = 3
        x_int = int(x)
        y_int = int(y)
            ...
            for i in range(max(0, y_int-cleaning_radius), min(self.map_height, y_int+cleaning_radius+1)):
                for j in range(max(0, x_int-cleaning_radius), min(self.map_width, x_int+cleaning_radius+1)):
                    ...
                    if distance <= cleaning_radius:
                        ...
                        reduction = 1.0 - (distance / cleaning_radius) * 0.7
                        self.mineral_map[i, j, :] *= reduction
        
        # Log minier
        mineral_at_pos = self.mineral_map[y_int, x_int, :]
        if np.max(mineral_at_pos) > 0.3:
            self.get_logger().info(
                f'⛏️ Mining at ({x_int}, {y_int}): '
    ...
    def update_system(self):
        ...
        with self.lock:
            # Évolution naturelle des dépôts
            if np.random.random() < 0.05:  # 5% de chance
                if self.generator_type == 'advanced':
                    # Évolution géologique lente
                    evolution = np.random.normal(0, 0.005, self.mineral_map.shape)
                    self.mineral_map = np.clip(self.mineral_map + evolution, 0, 1)
                else:
                    evolution = np.random.normal(0, 0.01, self.mineral_map.shape)
                    self.mineral_map = np.clip(self.mineral_map + evolution, 0, 1)
            
            # Régénération lente des zones nettoyées
            if np.random.random() < 0.1:
                regen_rate = 0.001
                self.mineral_map = np.minimum(1.0, self.mineral_map + regen_rate)
